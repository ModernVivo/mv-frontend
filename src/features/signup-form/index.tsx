...
import { doSignUp, doLogin } from '~/store/slices/authSlice';
...
export default function SignUpForm() {
  ...
  const onSubmit: SubmitHandler<SignUpRequest> = async (formData: SignUpRequest): Promise<void> => {
    const { username, password } = formData;

    setLoading(true);
    setError('root.apiError', { message: "" });
    await dispatch(doSignUp(formData))
      .unwrap()
      .then(async () => {
          // login 
      ...
      .catch(() => {
        setError('root.apiError', { message: "User Exist!" });
        setLoading(false);
      });
  }
            ...
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onSubmit)} autoComplete="off">
                ...
                <div>
                  ...
                  <input type="email" {...register('email')} placeholder="Email" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                  <div className="text-red-800 text-sm">{errors.email?.message}</div>
                </div>
                <div>
                  <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">First Name</label>
                  <input type="text"{...register('first_name')} placeholder="First Name" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                  <div className="text-red-800 text-sm">{errors.first_name?.message}</div>
                </div>
                <div>
                  <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Last Name</label>
                  <input type="text"{...register('last_name')} placeholder="Last Name" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                  <div className="text-red-800 text-sm">{errors.last_name?.message}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="password" {...register('confirmPassword')} placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                  <div className="text-red-800 text-sm">{errors.confirmPassword?.message}</div>
                </div>
